...
from openplantbook_sdk import MissingClientIdOrSecret, OpenPlantBookApi, ValidationError
...
try:
    ...
    print(
        "Copy config.yaml.dist to config.yaml and add client_id and secret from https://open.plantbook.io/apikey/show/"
    )
    sys.exit()
...
dti = pd.date_range(
    pd.Timestamp.now(tz="Australia/Sydney"), periods=NUMBER_OF_PERIODS, freq="15min"
)

# generate fake values - 4 columns to provide 4 values for the following measurements: temp, soil_moist, soil_ec, light_lux
df = pd.DataFrame(
    np.random.default_rng().uniform(100, 1000, (NUMBER_OF_PERIODS, 4)), index=dti
).astype(int)
...
try:
    res = asyncio.run(
        api.async_plant_instance_register(
            sensor_pid_map={SENSOR_ID: PID}, location_country="Australia"
        )
    )
